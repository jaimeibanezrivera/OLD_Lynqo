...
class CameraApp:
    ...
    # Custom dialog for entering image name (better for touch input)
    class CustomSaveDialog(tk.Toplevel):
        def __init__(self, parent, *args, **kwargs):
            super().__init__(parent, *args, **kwargs)
            self.transient(parent)  # Make the window modal
            self.grab_set()  # Ensure the focus remains on this window
            self.title("Save Image")

            # Label for the image name entry
            self.label = tk.Label(self, text="Enter a name for the image:")
            self.label.pack(pady=10)

            # Entry field for image name
        ...
        def on_ok(self):
            self.result = self.entry.get()  # Get the input from the entry
            self.destroy()
    ...
    class SaveImageDialog(QDialog):
        def __init__(self, parent=None):
            ...
            self.label = QLabel("Enter a name for the image:")
    ...
    def start_detection(self):
        ...
        if not self.resuming:
            ...
            self.camera_frame.grid(row=0, column=0, rowspan=2, padx=10, pady=5, sticky="nsew")            
