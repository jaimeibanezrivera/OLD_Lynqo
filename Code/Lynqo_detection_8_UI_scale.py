...
class CameraApp:
    ...
    def Nakijken_OK(self):
        ...
        button_width = self.screen_width // 10  # Adjust the width
        button_height = self.screen_height // 20  # Adjust to 1/15 of the screen height
        ...
        # Calculate button dimensions based on available screen size
        button_width = self.screen_width // 10  # Adjust the width
        button_height = self.screen_height // 15  # Adjust to 1/15 of the screen height
    ...
    def start_detection(self):
        ...
        if not self.resuming:
            ...
            self.main_frame.grid_columnconfigure(0, weight=2)  # Camera frame takes more space
            self.main_frame.grid_columnconfigure(1, weight=1)  # Labels frame takes less space
            self.main_frame.grid_rowconfigure(0, weight=4)     # Camera and labels take up more space
            ...
            # Make the columns and rows resize proportionally
            #self.main_frame.grid_columnconfigure(0, weight=1)  # Camera frame
            #self.main_frame.grid_columnconfigure(1, weight=1)  # Labels frame
            #self.main_frame.grid_rowconfigure(0, weight=1)
            
            # Create a frame for the buttons to arrange them horizontally
            self.button_frame = tk.Frame(self.main_frame, bg='white')
            self.button_frame.grid(row=1, column=0, columnspan=3, pady=5, sticky="ew")

            button_width = self.screen_width * 0.2  # Adjust this ratio for button size
            button_height = self.screen_height * 0.1
            
            # Create buttons and use relative placement
            self.create_rounded_button("Close Camera", self.close_camera, 200, 50, container=self.button_frame, Side=tk.LEFT, X=35)
            self.create_rounded_button("Save Image", self.toggle_pause, 200, 50, container=self.button_frame, Side=tk.LEFT, X=30)
            self.create_rounded_button("Mark Incomplete", self.mark_incomplete, 200, 50, container=self.button_frame, Side=tk.LEFT, X=35)

            # Make sure buttons expand horizontally
            self.button_frame.grid_columnconfigure(0, weight=7)
