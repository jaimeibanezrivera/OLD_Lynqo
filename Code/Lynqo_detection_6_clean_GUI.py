...
class CameraApp:
    def __init__(self, window):
        ...
        self.model_path = 'ssd_mobilenet_v2_coco.tflite'  # Path to TFLite model
        self.labels_path = 'mscoco_label_map.txt'  # Path to label names for COCO dataset
    ...
    def load_icons(self):
        ...
        self.home_image = Image.open("Images_GUI/home.png")
        ...
        self.quit_image = Image.open("Images_GUI/quit.png")
        ...
        self.sleep_image = Image.open("Images_GUI/sleep.png")
    ...
    def Nakijken_OK(self):
        ...
        # Create a frame to hold the buttons (this helps with centering them)
        buttons_outer_frame = tk.Frame(button_container, bg="white")
        buttons_outer_frame.pack(anchor='center')

        # Populate buttons based on the sets available
        for i, set_name in enumerate(sets):
            # Create a new row frame for every second button
            if i % num_columns == 0:
                row_frame = tk.Frame(buttons_outer_frame, bg="white")
                row_frame.grid(row=i // num_columns, column=0, padx=20, pady=10, sticky="nsew")

            # Create buttons and place them inside the row frame using grid for symmetrical alignment
        ...
        # Button for starting object detection, still using pack here as it's outside the grid-managed container
