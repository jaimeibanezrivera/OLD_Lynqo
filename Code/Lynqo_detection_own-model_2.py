...
class CameraApp:
    def __init__(self, window):
        ...
        self.sets = self.load_sets('Code/sets_lynqo.json')

        self.label = tk.Label(window, bg="white")
        self.label.pack()

        # Load object detection model and labels
        self.model_path = 'Code/model.tflite'  # Path to TFLite model
        self.labels_path = 'Code/lynqo_label_map.txt'  # Path to label names for COCO dataset
        ...
        #already call the onboard keyboard for setup, will be called in the save_image function
    ...
    def start_detection(self):
        ...
        if not self.resuming:
            ...
            if self.camera is None:
                ...
                self.camera.configure(self.camera.create_preview_configuration(main={"format": "RGB888", "size": full_resolution}, controls={"FrameRate": 15}, transform=Transform(hflip=1, vflip=1)))
                self.camera.start()
    ...
    def detect_objects(self):
        if self.camera:
            if not self.paused:  # Check if the camera is not paused
                ...
                image_rgb = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)  # Convert BGR to RGB for PIL
