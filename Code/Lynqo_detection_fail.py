...
class CameraApp:
    ...
    def start_detection(self):
        ...
        if self.detect_thread is None or not self.detect_thread.is_alive():
            self.detect_thread = threading.Thread(target=self.detect_objects, daemon=True)
    ...
    def save_image(self):
        ...
        # Capture the current frozen frame
        image = self.camera.capture_array()
        image_rgb = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)  # Convert BGR to RGB
    ...
    def resume_detection(self):
        """Resume real-time detection by unfreezing the image."""
    ...
    def close_camera(self):
        """Close the camera and reset the interface."""
        if self.camera:
            self.camera.stop()
            self.camera.close()
            self.camera = None
